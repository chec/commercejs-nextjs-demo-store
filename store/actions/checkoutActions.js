...
export const getShippingOptionsForCheckout = (checkoutId, country = 'US') => (dispatch) => commerce.checkout.getShippingOptions(checkoutId, { country })
...
export const generateCheckoutTokenFromCart = (cartId) => (dispatch) => commerce.checkout.generateToken(cartId, { type: 'cart' }).then((checkout) => {
  dispatch({
...
export const setShippingOptionInCheckout = (checkoutId, shippingOptionId, country, region) => (dispatch) => commerce.checkout.checkShippingOption(checkoutId, {
...
export const setDiscountCodeInCheckout = (checkoutId, code) => (dispatch) => commerce.checkout.checkDiscount(checkoutId, { code })
  .then((resp) => {
    dispatch({
      type: UPDATE_CHECKOUT_LIVE_OBJECT,
      payload: resp.live,
...
export const captureOrder = (checkoutId, order) => (dispatch) => commerce.checkout.capture(checkoutId, order)
  .then((resp) => {
